...
class UserDBHelper(var context: Context,private var DB_VERSION:Int = CUURRENT_VERSION):ManagedSQLiteOpenHelper(context,
    DB_NAME,null,DB_VERSION) {
    companion object{
        private val TAG = "UserDBHelper"
        var DB_NAME = "user.db"//数据库名称
        var TABLE_NAME = "user_info"//表名称
        var CUURRENT_VERSION = 1//当前最新版本，如有表结构变更，该版本号要加一
        ...
        fun getINstance(ctx:Context,version :Int =0):UserDBHelper{
            if (instance == null){
                ...
                instance = if (version >0) UserDBHelper(ctx.applicationContext,version) else UserDBHelper(ctx.applicationContext)
            }
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        ...
        if (newVersion >1){
            ...
            var alter_sql = "ALTER TABLE$TABLE_NAME ADD COLUMN phone VARCHAE"
    ...
    fun insert(info: UserInfo):Long{
     var infoArray = mutableListOf(info)
